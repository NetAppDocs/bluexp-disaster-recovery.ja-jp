...
* *オンプレミスからオンプレミスのFC向けVMware VMFSデータストアのサポート*：このリリースには、FCでオンプレミスストレージに保護されたVMware vSphere Virtual Machine File System（VMFS）データストアにマウントされたVMの_テクノロジプレビュー_サポートが含まれています。以前は、このサービスでは、iSCIのVMFSデータストアをサポートするテクノロジプレビューが提供されていました。このリリースでは、BlueXPディザスタリカバリで、FC用のVMFSデータストアを使用するオンプレミスのVMwareワークロードからオンプレミスのVMware環境へのテクノロジプレビューがサポートされます。
+
